...
public class TabWebContentsDelegateAndroid extends WebContentsDelegateAndroid {
    ...
    public void toggleFullscreenModeForTab(boolean enableFullscreen) {
        ...
        if (!enableFullscreen) {
            ...
            if (mOriginalBrightness != INVALID_BRIGHTNESS) {
                ...
                nativeOnMediaBrightnessChanged(mTab.getWebContents(), mOriginalBrightness);
            }
    ...
    @CalledByNative
    private void mediaAdjustBrightness(float delta) {
        Activity activity = mTab.getActivity();
        if (activity == null)
            return;
        ...
        if (lp.screenBrightness == -1) {
            try {
                brightness = Settings.System.getInt(
                        ((Context) activity).getContentResolver(),
        ...
        nativeOnMediaBrightnessChanged(mTab.getWebContents(), brightness);
    }

    @CalledByNative
    private void mediaSetRotateLock(boolean lock) {
        Activity activity = mTab.getActivity();
        if (activity == null)
            return;
    ...
    private static native void nativeOnMediaBrightnessChanged(
            WebContents webContents, float brightness);
}
